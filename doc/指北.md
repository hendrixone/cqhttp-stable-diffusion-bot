...
默认使用512 * 512像素输出，支持使用额外参数生成更高分辨率图片，但耗时会显著增加。
...
SD1.5, MeinaMix/Pastel, Dark Sushi Mix, TMND-Mix, Anything,等等

现阶段只支持一次运行一个底模，如何切换是个值得商榷的问题，因为切换模型耗时太长（200秒）。
...
想要更多定制内容？可以直接私聊机器人，私聊模式不会有任何内容限制

毕竟只是个娱乐项目，真的对画图感兴趣就自行去了解吧

### 图生图（重绘图片）
觉得某一张图片不错？可以尝试使用重绘功能，重绘功能支持使用不同分辨率重绘图片。
...
后面的数字是图片生成是对应的ID，过久之前的图片将无法被重绘。自行上传图片功能还未被开放。
